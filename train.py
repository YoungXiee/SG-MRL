...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size, last_test=False):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        ...
        #在最后一个batch进行维度补全
        target_size = [batch_size, 1, args.num_nodes, args.seq_in_len]
        current_size = list(X.size())
        padding_size = target_size[0] - current_size[0]
        if padding_size > 0:
            pad = torch.zeros(padding_size, *current_size[1:], dtype=X.dtype, device= X.device)
            X = torch.cat([X, pad], dim=0)
        target_size_y = [batch_size, args.num_nodes]
        current_size_y = list(Y.size())
        padding_size_y = target_size_y[0] - current_size_y[0]
        if padding_size_y > 0:
            pad = torch.zeros(padding_size_y, *current_size_y[1:], dtype=Y.dtype, device=Y.device)
...
def evaluate1(data, X, Y, model, evaluateL2, evaluateL1, batch_size, last_test=False):
    model.eval()
    total_loss = 0
    total_loss_l1 = 0
    n_samples = 0
    ...
    return rse, rae, correlation, mae, rmse, predict, Ytest
...
parser.add_argument('--K',type=int,default=32,help='number of Hyperedges')
parser.add_argument('--batch_size',type=int,default=4,help='batch size')
parser.add_argument('--lr',type=float,default=0.0001,help='learning rate')
parser.add_argument('--weight_decay',type=float,default=0.00001,help='weight decay rate')
parser.add_argument('--clip',type=int,default=5,help='clip')
parser.add_argument('--propalpha',type=float,default=0.01,help='prop alpha')
parser.add_argument('--tanhalpha',type=float,default=5,help='tanh alpha')
...
def main(params):
    ...
    model = magnn(args.gcn_depth, args.K, args.num_nodes,
    ...
    vtest_acc, vtest_rae, vtest_corr, vtest_mae, vtest_rmse = evaluate(Data, Data.valid[0], Data.valid[1], model, evaluateL2, evaluateL1,
                                         args.batch_size)
    test_acc, test_rae, test_corr, test_mae, test_rmse, predict, Ytest = evaluate1(Data, Data.test[0], Data.test[1], model, evaluateL2, evaluateL1,
                                         args.batch_size)
    ...
    num_variable = predict.shape[1]
    x_time = np.arange(predict.shape[0])
    for i in range(num_variable):
        plot_prediction(x_time, predict[:,i], Ytest[:,i], i)
...
def plot_prediction(time, predict, ytest, i):
    plt.plot(time, predict, label= "prediction")
    plt.plot(time, ytest, label = 'Actual')
    plt.xlabel("Time")
    plt.ylabel("Value")
    plt.legend()
    plt.savefig(f'./result_image/predic_vs_actual_variable_{i+1}.png')
    plt.clf()
